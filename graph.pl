...
`yacc yacc.y 2>error.txt`; #compile lex to generate y.tab.c
`lex lex.l > error.txt`; #compile lex to generate lex.yy.c
`cc lex.yy.c  y.tab.c -ly -ll 2> error.txt`; #generates a.out
